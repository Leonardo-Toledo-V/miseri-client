import React from 'react'
import Link from 'next/link'

export default function FormRegister() {
    return (
        <>
            <div className="h-full w-full p-4 sm:p-16 max-w-[900px] flex flex-col">
                <div className="sm:mx-16 mt-16 flex flex-col">
                    <div className='mb-12'>
                        <h3 className="font-playfair text-4xl">Crea tu cuenta</h3>
                        <h4 className="font-lato text-lg">Empieza a descubrir las maravillas de Miseri.</h4>
                    </div>
                    <button
                        type="submit"
                        className="mt-4 font-lato text-[0.920rem] border-[0.1px] w-full hover:border-b-2 py-1 duration-300 cursor-pointer flex items-center justify-center"
                    >
                        <span>
                            <img className="h-3 w-3 sm:h-5 sm:w-5 mx-4" src="google.png" alt="google" />
                        </span>
                        Ingresar con Google
                    </button>
                    <form>
                        <div className="flex flex-col mt-4">
                            <div className="flex items-center justify-center mt-4 ">
                                <hr className="w-52" />
                                <p className="font-lato text-[#656565] mx-4">o</p>
                                <hr className="w-52" />
                            </div>
                            <label
                                htmlFor="email"
                                className="font-lato font-black mb-2 text-sm text-[#656565]"
                            >
                                Correo electrónico:
                            </label>
                            <input
                                className="bg-[#F3F6FB] py-1.5 pl-2 text-sm text-left"
                                type="text"
                                id="email"
                                placeholder="Ingresa tu correo electrónico"
                                autoComplete="off"
                                required
                            />
                        </div>
                        <div className="flex flex-col mt-4">
                            <label
                                htmlFor="email"
                                className="font-lato font-black mb-2 text-sm text-[#656565]"
                            >
                                Contraseña:
                            </label>
                            <input
                                className="bg-[#F3F6FB] py-1.5 pl-2 text-sm text-left"
                                type="password"
                                id="email"
                                placeholder="Crea una contraseña"
                                autoComplete="off"
                                required
                            />
                        </div>

                        <div className="flex justify-between mb-8 mt-2">
                            <div className="flex items-center">
                                <label
                                    className="text-sm font-lato text-[#656565]"
                                >
                                    Debe de contener al menos 8 caracteres *
                                </label>
                            </div>
                        </div>

                        <div>
                            <input className="bg-[#373737] text-white font-lato w-full py-1 hover:bg-[#4e4e4e] duration-300 cursor-pointer" type="submit" />
                        </div>
                    </form>
                </div>
                <div className="sm:mx-16 mt-2">
                    <div className="flex justify-center items-center font-lato text-sm mt-4">
                        <div className="mx-2 text-[#656565]">
                            <p>¿Ya tienes una cuenta?</p>
                        </div>
                        <div className="mx-2 text-[#373737] cursor-pointer hover:border-b-2 duration-300">
                            <Link href="/login">
                                <p>Ingresar</p>
                            </Link>
                        </div>
                    </div>
                </div>
            </div>
        </>
    )
}
